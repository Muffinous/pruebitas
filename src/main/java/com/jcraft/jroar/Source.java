...
class Source {
  ...
  Vector<Client> listeners = new Vector<Client>();
  String mountpoint;
  String source = null;
  ...
  void parseHeader(Page[] pages, int count){
    current_info.rate=0;
      ...
      for(int i=0; i<count; i++){
        ...
        if(foo[base+0] == 'v' && foo[base+1] == 'o' && foo[base+2]=='r' && foo[base+3]=='b' && foo[base+4]=='i' && foo[base+5]=='s'){
          key_serialno = serialno;
          current_info=vi = (vis.get(serialno));
          vc = (vcs.get(serialno));
          vi.synthesis_headerin(vc, op);
        } else if (foo[base-1+0]=='S' && foo[base-1+1]=='p' && foo[base-1+2]=='e' && foo[base-1+3]=='e' && foo[base-1+4]=='x' && foo[base-1+5]==' ' && foo[base-1+6]==' ' && foo[base-1+7]==' '){
          key_serialno=serialno;
          current_info=vi=(vis.get(serialno));
            if(vi.rate==0){
                vi.rate=(((foo[base-1+39]<<24)&0xff000000)| ((foo[base-1+38]<<16)&0xff0000)| ((foo[base-1+37]<<8)&0xff00)| ((foo[base-1+36])&0xff));
	        }
          }
      }
  }
  ...
  //System.out.println(new String(buffer, 28, 8)); // "Speex   "
    ...
    if((buffer.length<base+8)||
       (buffer[base+0]!='S' || buffer[base+1]!='p' || buffer[base+2]!='e' || buffer[base+3]!='e' || buffer[base+4]!='x' || buffer[base+5]!=' ' || buffer[base+6]!=' ' || buffer[base+7]!=' ')){
      return;
    }
